...
export default async function handler(req, res) {
  ...
  // Admin faucet: transfer SQUDY from treasury/relayer to a target address
  if (req.method === 'POST' && /^\/(?:api\/)?admin\/faucet\/?$/.test(url)) {
    try {
      const enabled = String(process.env.FAUCET_ENABLED || 'false').toLowerCase() === 'true';
      if (!enabled) {
      ...
      const amountStr = String(body?.amount || '0').trim();
      const maxAmount = Number(process.env.FAUCET_MAX_AMOUNT || '1000');
      const dailyCap = Number(process.env.FAUCET_DAILY_CAP || '10000');
      ...
      if (amountNum > maxAmount) {
        return res.status(400).json({ error: `Amount exceeds faucet max (${maxAmount})` });
      }

      // Rate limiting: per address per day and total daily cap
      const db = await getDb();
      const logs = db.collection('faucet_logs');
      const today = new Date();
      today.setUTCHours(0, 0, 0, 0);
      const startIso = today.toISOString();
      const nowIso = new Date().toISOString();

      const [addrTotalToday, platformTotalToday] = await Promise.all([
        logs.aggregate([
          { $match: { to, createdAt: { $gte: startIso } } },
      ...
      const perAddrSoFar = addrTotalToday[0]?.total || 0;
      const platformSoFar = platformTotalToday[0]?.total || 0;
      if (perAddrSoFar + amountNum > maxAmount) {
        return res.status(429).json({ error: `Per-address daily limit reached (${maxAmount})` });
      }
      if (platformSoFar + amountNum > dailyCap) {
        return res.status(429).json({ error: 'Faucet daily cap reached' });
      }

      // Signer
      const pk = process.env.TREASURY_PRIVATE_KEY || process.env.RELAYER_PRIVATE_KEY;
      const rpc = process.env.RPC_URL || process.env.VITE_RPC_URL;
      const token = process.env.VITE_SQUDY_TOKEN_ADDRESS;
      const chainId = Number(process.env.CHAIN_ID || process.env.VITE_CHAIN_ID || 11155111);
      if (!pk || !rpc || !token) {
      ...
      if (bal.lt(amountWei)) {
        return res.status(400).json({ error: 'Treasury has insufficient SQUDY balance' });
      }
      ...
      // Log
      await logs.insertOne({ to, amount: amountNum, txHash: tx.hash, createdAt: nowIso });

      res.setHeader('Cache-Control', 'no-store');
      return res.json({ success: true, txHash: tx.hash });
    } catch (e) {
      console.error('POST /admin/faucet error:', e);
      return res.status(500).json({ error: 'Internal Server Error' });
    }
  }
