...
async function main() {
  ...
  try {
    ...
    await mockSqudyToken.deployTransaction.wait(5);
    await squdyCampaignManager.deployTransaction.wait(5);
    ...
    envContent = envContent.replace(
      /SQUDY_TOKEN_ADDRESS=.*/,
      `SQUDY_TOKEN_ADDRESS=${mockSqudyToken.address}`
    );

    // Update CAMPAIGN_MANAGER_ADDRESS
    envContent = envContent.replace(
      /CAMPAIGN_MANAGER_ADDRESS=.*/,
      `CAMPAIGN_MANAGER_ADDRESS=${squdyCampaignManager.address}`
    );
    ...
    const networkName = network.chainId === 97 ? "testnet" : "mainnet";
    envContent = envContent.replace(
      /NETWORK=.*/,
      `NETWORK=${networkName}`
    );
    ...
    const deploymentSummary = {
      ...
      contracts: {
        ...
        campaignManager: {
          address: squdyCampaignManager.address,
    ...
    console.log("\n🔗 Contract Links:");
    const explorerBase = network.chainId === 97 
      ? "https://testnet.bscscan.com" 
      : "https://bscscan.com";
    console.log(`SQUDY Token: ${explorerBase}/address/${mockSqudyToken.address}`);
    console.log(`Campaign Manager: ${explorerBase}/address/${squdyCampaignManager.address}`);
